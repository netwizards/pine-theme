...
function roots_gallery_shortcode($attr) {
  ...
  $gallery_div = "<section id='$selector' class='thumbnails gallery galleryid-{$id} gallery-columns-{$columns} gallery-size-{$size_class}'>";
  ...
  foreach ($attachments as $id => $attachment) {
    ...
    if ($columns > 0 && $i % $columns == 0)
      ...
      $add_class = '';
    $output .= "
      <{$icontag} class=\"gallery-item $add_class \">
        $link
      ";
    if ($captiontag && trim($attachment->post_excerpt)) {
      $output .= "
        <{$captiontag} class=\"gallery-caption ";
			if($caption_hidden) $output .="hidden";
